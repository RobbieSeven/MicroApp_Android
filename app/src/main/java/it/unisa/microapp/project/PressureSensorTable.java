 ...
 */
public class PressureSensorTable {
    SQLiteDatabase mDb;
    static DatabaseHelper helper;
    Context context;

    // Database table
    public static final String TABLE_SENSOR = "pressure_sensor";
...
//    public static final String NAME_PROJECT = "name_project";
    ...
    public PressureSensorTable(Context context) {
...
//            + NAME_PROJECT + " text, "
    ...
    static class MetaData {  // i metadati della tabella, accessibili ovunque
        static final String TABLE_SENSOR = "pressure_sensor";
...
//        static final String NAME_PROJECT = "name_project";
    ...
    public void insertUser(String data, String value){ //metodo per inserire i dati
        ContentValues cv=new ContentValues();
//        cv.put(MetaData.NAME_PROJECT, name_project);
        cv.put(MetaData.DATA, data);
        cv.put(MetaData.VALUE, value);
...
//        mDb=helper.getReadableDatabase();
    ...
    public static ArrayList<PressureSensorObject> getValues(Context context, long from, long to, String sensor){
        SQLiteDatabase db = helper.getReadableDatabase();

        String QUERY = "SELECT * FROM "+ PressureSensorTable.TABLE_SENSOR ;
//        + " WHERE " + PressureSensorTable.NAME_PROJECT + "=" + "'" + filename + "'";
        Cursor c = db.rawQuery(QUERY, null);
        Log.d("info" , "size db " + c.getCount());
        c.moveToFirst();
        ArrayList<PressureSensorObject> toReturn = new ArrayList<PressureSensorObject>();

        while(c.isAfterLast()==false){

            String value,date;

            value = c.getString(c.getColumnIndex(PressureSensorTable.VALUE));
            date = c.getString(c.getColumnIndex(PressureSensorTable.DATA));

            PressureSensorObject toAdd = new PressureSensorObject(value, date);
